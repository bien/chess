...
FenBoard::FenBoard()
	: side_to_play(White), enpassant_file(-1), move_count(0), castle(0x0f), hash(0)
{}
...
int FenBoard::get_castle_bit(Color color, bool kingside) const
{
	int bit = 0;
	if (kingside) {
		bit += 1;
...
bool FenBoard::can_castle(Color color, bool kingside) const
{
	int bit = get_castle_bit(color, kingside);
	return castle & (1 << bit);
}

void FenBoard::apply_move(move_t move)
{
	...
	if ((sourcepiece & PIECE_MASK) == KING) {
		...
		if (sourcefile == 4) {
			...
			piece_t castlerook = ROOK | (color == Black ? BlackMask : 0);
...
void FenBoard::undo_move(move_t move)
{
	unsigned char destrank, destfile, sourcerank, sourcefile;
	get_source(move, sourcerank, sourcefile);
	get_dest(move, destrank, destfile);
	...
	if (captured_piece != EMPTY) {
		captured_piece = set_color(captured_piece, get_opposite_color(color));
	}
...
//	in_check = move & MOVE_FROM_CHECK;
	...
	if ((moved_piece & PIECE_MASK) == KING && sourcefile == 4) {
		...
		set_piece(rook_source_file, destrank, rook);
		set_piece(rook_dest_file, destrank, EMPTY);
		update_zobrist_hashing_piece(rook_source_file, destrank, rook, true);
		update_zobrist_hashing_piece(rook_dest_file, destrank, rook, false);
	}
	...
	if (move & ENPASSANT_FLAG) {
		set_piece(sourcerank, destfile, make_piece(PAWN, get_opposite_color(color)));
	}
...
void FenBoard::update_zobrist_hashing_piece(unsigned char rank, unsigned char file, piece_t piece, bool adding)
{
	std::cout << "update_zobrist_hashing_piece " << (int)rank << (int)file << " " << (int)piece << " " << adding << std::endl;
...
void FenBoard::update_zobrist_hashing_move()
{
	std::cout << "update_zobrist_hashing_move" << std::endl;
	hash ^= zobrist_hashes[64*6*2];
}
void FenBoard::update_zobrist_hashing_castle(Color color, bool kingside, bool enabling)
{
	int index = 0;
	if (color == Black) {
		index += 1;
	...
	std::cout << "update_zobrist_hashing_castle " << kingside << " " << color << " " << index << " " << enabling << std::endl;
//	hash ^= zobrist_hashes[64*6*2 + 1 + index];
}
void FenBoard::update_zobrist_hashing_enpassant(int file, bool enabling)
{
	assert(file >= 0 && file < 8);
	std::cout << "update_zobrist_hashing_enpassant " << file << " " << enabling << std::endl;
	hash ^= zobrist_hashes[64*6*2 + 1 + 4 + file];
}

uint64_t FenBoard::zobrist_hashes[] = {0xe02111c7659eae86L, 0x6da4e7350d2011b3L, 0x83cd50fcc552e5e9L, 0x78615f6439c6c6b4L, 0x51c92cc23497add2L,
