...
void SimpleBoard::apply_move(move_t move)
{
	...
	if ((sourcepiece & PIECE_MASK) == KING) {
		...
		if (get_board_file(sourcepos) == 4) {
			BoardPos rooksource = InvalidPos;
			BoardPos rookdest = InvalidPos;
			piece_t castlerook = ROOK | (color == Black ? BlackMask : 0);

			switch (get_board_file(destpos)) {
			case 2:
				rooksource = make_board_pos(get_board_rank(sourcepos), 0);
				rookdest = make_board_pos(get_board_rank(sourcepos), 3);
				break;
			case 6:
				rooksource = make_board_pos(get_board_rank(sourcepos), 7);
				rookdest = make_board_pos(get_board_rank(sourcepos), 5);
				break;
			}
			if (rooksource != InvalidPos) {
				set_piece(rookdest, castlerook);
				set_piece(rooksource, EMPTY);
				update_zobrist_hashing_piece(rookdest, castlerook, true);
				update_zobrist_hashing_piece(rooksource, castlerook, false);
			}
		}
	}
		...
		// enpassant capture
			...
			BoardPos deleted_pawn_pos = make_board_pos(get_board_rank(sourcepos), get_board_file(destpos));
			update_zobrist_hashing_piece(deleted_pawn_pos, get_piece(deleted_pawn_pos), false);
			set_piece(deleted_pawn_pos, EMPTY);
		}
	}
...
void SimpleBoard::undo_move(move_t move)
{
	...
	if ((moved_piece & PIECE_MASK) == KING && get_board_file(sourcepos) == 4) {
		...
		BoardPos rooksrc = InvalidPos, rookdest = InvalidPos;
		switch (get_board_file(destpos)) {
			case 2:
				rooksrc = make_board_pos(get_board_rank(destpos), 0);
				rookdest = make_board_pos(get_board_rank(destpos), 3);
				break;
			case 6:
				rooksrc = make_board_pos(get_board_rank(destpos), 7);
				rookdest = make_board_pos(get_board_rank(destpos), 5);
				break;
		}
		set_piece(rooksrc, rook);
		set_piece(rookdest, EMPTY);
...
void SimpleBoard::update_zobrist_hashing_piece(BoardPos pos, piece_t piece, bool adding)
{
	...
	if (pt >= 0) {
		int posindex = get_board_file(pos) + get_board_rank(pos) * 8;
//		std::cout << std::hex << "pp " << posindex + pt * 64 + color * (64 * 6) << " " << "x NBRQKx"[piece & 0x7] << static_cast<char>(get_board_file(pos) + 'a') << static_cast<char>(get_board_rank(pos) + '1') << ": (" << zobrist_hashes[posindex + pt * 64 + color * (64 * 6)] << ") " << color * (64 * 6) << ": " << hash << " ";
